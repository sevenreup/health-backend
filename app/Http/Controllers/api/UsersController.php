...
class UsersController extends Controller
{
    ...
    {
        $contacts = contactTraceUser::select('id','sender')->where('sender',Auth::user()->id)->orWhere('recipient',Auth::user()->id)->where('status','accepted')->with('User:id,first_name,last_name,phone')->get();
        return $contacts;
    }
    ...
    {
        $contacts = contactTraceUser::select('id','sender')->where('recipient',Auth::user()->id)->where('status','pending')->with('User:id,first_name,last_name,phone')->get();
        return $contacts;
    }
